...
- `symlinkLibrary+Application Support+nushell+config.nu`: entrypoint loaded by Nu. Resolves this directory and sources `history.nu`, `hotkeys.nu`, and `prompt.nu`.
...
- `prompt.nu`: Starship/Nushell prompt integration. It sets `STARSHIP_CONFIG`, installs left and right prompt closures, and formats command duration for Starship.
- `starship.toml`: Starship prompt layout and styling for cwd, git, language tools, duration, cloud/IP info, and timestamp.
